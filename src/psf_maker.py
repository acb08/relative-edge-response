...
class PSF(object):
    ...
    def __init__(self, num_samples, f_num, pix_pitch, ff, wl):
        ...
        self.psf = None  # PSF of circular aperture w/o central obscuration. Used for PSF scaling
    ...
    def get_zero(self):
        zeros = np.int64(self.zcr(self.N_set[self.num_samples // 2:self.num_samples // 2 + 300],
                                  self.psf[self.num_samples // 2, self.num_samples // 2:self.num_samples // 2 + 301])) + 1 # Finding 5th zero index
        x = np.array([zeros[0] - 1, zeros[0]])
        y = np.array([self.psf[self.num_samples // 2, x[0] + self.num_samples // 2], self.psf[self.num_samples // 2, x[1] + self.num_samples // 2]])
    ...
    def scaling(self):
        ...
        self.psf = np.real(self.fft2(self.open_apt)) # Coherent PSF of unobscured aperture
        self.zero = self.get_zero() # Find 5th zero w/ sub pixel accuracy
        self.cal_scale = (1/self.zero)*(3.8317/np.pi)*(self.wl * self.f_num) # Scale 1st zero with system parameters
    ...
    def mtf_aperture(self):
        if self.ff != 0:  # shouldn't this be if self.ff != 1?
            ...
            self.aperture_mtf = self.psf2mtf(abs(self.psf) ** 2)  # Incoherent PSF requires the squaring
    ...
    def create_uprf(self, sub_shiftx=None, sub_shifty=None, sub_sample=None):
        ...
        if (sub_shiftx==None) & (sub_shifty==None) & (sub_sample==None):
        ...
        if (sub_shiftx != None) & (sub_shifty != None):
